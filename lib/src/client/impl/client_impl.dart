...
class _ClientImpl implements Client {
  ...
  Future _reconnect() {
    ...
    if (settings.tlsContext != null) {
      connectionLogger.info("Trying to connect to ${settings.host}:${settings.port} using TLS [attempt ${_connectionAttempt + 1}/${settings.maxConnectionAttempts}]");
      fs = SecureSocket.connect(settings.host, settings.port, context: settings.tlsContext);
    } else {
